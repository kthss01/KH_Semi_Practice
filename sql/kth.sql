...
SELECT PROJECT_CODE FROM (SELECT * FROM PROJECT ORDER BY PROJECT_CODE DESC) WHERE ROWNUM = 1;

SELECT * FROM PROJECT WHERE PRICE = 0;

-- FAQ 테이블 주석 붙이기
COMMENT ON COLUMN FAQ.F_NO IS '글번호';
COMMENT ON COLUMN FAQ.TARGET_USER IS '타겟유저';
COMMENT ON COLUMN FAQ.QUESTION IS '질문내용';
COMMENT ON COLUMN FAQ.ANSWER IS '답변내용';
COMMENT ON COLUMN FAQ.CREATOR_ID IS '작성자이메일';
COMMENT ON COLUMN FAQ.CREATE_DATE IS '작성일자';
COMMENT ON COLUMN FAQ.UPDATER_ID IS '수정자이메일';
COMMENT ON COLUMN FAQ.UPDATE_DATE IS '수정일자';
COMMENT ON COLUMN FAQ.SHOW_YN IS '게시여부';
